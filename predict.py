...
def predict(model, PredLoader, val_transforms, args):
    save_dir = 'out/'
    ...
    for index, batch in enumerate(tqdm(PredLoader)):
...
def main():
    ...
    pred_loader, val_transforms = get_loader(args)

    predict(model, pred_loader, val_transforms, args)
