...
def get_loader(args):
    ...
    if args.phase == 'predict': # based off of 'test' section above
        pred_img = []
        pred_lbl = []
        pred_post_lbl = []
        pred_name = []
        ...
        data_dicts_pred = [{'image': image, 'label': label, 'post_label': post_label, 'name': name}
                    for image, label, post_label, name in zip(pred_img, pred_lbl, pred_post_lbl, pred_name)]
        print('pred len {}'.format(len(data_dicts_pred)))

        if args.cache_dataset:
            pred_dataset = CacheDataset(data=data_dicts_pred, transform=val_transforms, cache_rate=args.cache_rate)
        else:
            pred_dataset = Dataset(data=data_dicts_pred, transform=val_transforms)
